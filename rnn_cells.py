...
def rnn_cell(x, h, units, scope='rnn_cell', 
    ...
    with tf.variable_scope(scope):
        x_dim = shape_list(inputs)[1]
        w = tf.get_variable("w", [x_dim, units], initializer=w_init)
...
def lstm_cell(x, c, h, units, scope='lstm_cell', 
    ...
    with tf.variable_scope(scope):
        ...
        w = tf.get_variable("w", [x_dim, units * 4], initializer=w_init)
        b = tf.get_variable("b", [units * 4], initializer=b_init)
        x = _rnn_dropout(x, i_kp)
        z = tf.matmul(tf.concat([x, h], 1), w) + b
        i, j, f, o = tf.split(z, 4, 1)
        c = tf.nn.sigmoid(f + fb) * c + tf.nn.sigmoid(i) * tf.tanh(j)
...
def peep_lstm_cell(x, c, h, units, scope='peep_lstm_cell', 
    ...
    with tf.variable_scope(scope):
        ...
        c = tf.sigmoid(f + f_b + w_f_diag * c) * c + sigmoid(i + w_i_diag * c) * tf.tanh(j)
...
def l2_mlstm_cell(x, c, h, units, scope='peep_mlstm_cell', 
    w_init=tf.random_normal_initializer(stddev=0.02), 
    b_init=tf.constant_initializer(0),
    f_b=1.0, i_kp=1.0, o_kp=1.0):
    with tf.variable_scope(scope):
        ...
        c = tf.nn.sigmoid(f + fb) * c + tf.nn.sigmoid(i) * tf.tanh(u)
        h = tf.nn.sigmoid(o) * tf.tanh(c)
        h = _rnn_dropout(h, o_kp)
        return h, c
...
def gru_cell(x, h, units, scope='gru_cell', 
    ...
    with tf.variable_scope(scope):
        x_dim = shape_list(x)[1]
        w_g = tf.get_variable("w_g", [x_dim, units], initializer=w_init)
        b_g = tf.get_variable("b_g", [units], initializer=b_init)
        w_c = tf.get_variable("w_c", [x_dim, units], initializer=w_init)
        b_c = tf.get_variable("b_c", [units], initializer=b_init)
        x = _rnn_dropout(x, i_kp)
        g = tf.sigmoid(tf.matmul(tf.concat([x, h], 1), w_g)+b_g)
